...
- No sidecar: the DAG engine and durable execution (in-process `FallbackEngine`)
  live in Core's workflow module. Core owns local schedules and workflow target
  authorization; when Connect is configured, Connect owns provider trigger
  creation, ingress, and delivery and Core stores only the target binding.
...
When Core reports Connect as the Composio execution owner, the trigger editor
uses the host bridge to list, bind, and remove Connect trigger IDs. Connect owns
provider trigger creation, ingress, and delivery; Core owns workflow target
authorization and execution. Resaving a workflow preserves a Connect binding.

## Star History

<a href="https://github.com/amajorai/ryu-workflows/stargazers">
  <picture>
