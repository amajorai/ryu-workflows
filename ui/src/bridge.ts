...
export type WorkflowTrigger =
	 ...
	 * manifest `contributes.hook_events` and raised at runtime. `event` is the
	 * fully-qualified id, `<owning plugin id>#<event name>`.
	 *
	 * Subscribing to an event whose provider is not installed is deliberately
	 * allowed: it simply never fires until the app is installed, which is what lets
	 * a user build the automation before the app that feeds it.
	 */
...
export interface NotifyTargetUser {
	email: string | null;
	id: string;
	image: string | null;
	name: string;
...
/** Read the shared Ryu provider/agent catalog used by the node pickers. */
...
/** Read the verified organization roster used by NotifyUser's multi-person picker. */
export function fetchNotifyTargetUsers(): Promise<NotifyTargetUser[]> {
	return ryu().workflows.notifyTargets() as Promise<NotifyTargetUser[]>;
}
...
/** One app event an enabled app declares it emits — the catalog behind the `event`
 *  trigger's picker. */
export interface HookEventInfo {
	description?: string;
	/** Fully-qualified event id: `<owning plugin id>#<event name>`. */
	id: string;
...
export function fetchHookEvents(_t?: ApiTarget): Promise<HookEventInfo[]> {
	return ryu().workflows.hookEvents() as Promise<HookEventInfo[]>;
}
