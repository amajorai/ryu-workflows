import { RyuAppToolbar } from "@ryu/blocks/companion/app-ui";
import {
	isViewVisible,
...
function ResumePanel({
	...
	return (
		<div className="bg-warning/10 px-4 py-3">
			<p className="mb-2 font-medium text-sm text-status-warning">
				{isNotificationGate
					? "Awaiting approvals"
			...
			{err ? (
				<p aria-live="polite" className="mt-1 text-status-destructive text-xs">
					{err}
				</p>
			) : null}
		</div>
...
export function App() {
	...
	const selectedWorkflowId = selected?.id;
	useEffect(() => {
		if (!(selectedWorkflowId && awaitingRunId)) {
			return;
		}
	...
	}, [awaitingRunId, selectedWorkflowId]);
