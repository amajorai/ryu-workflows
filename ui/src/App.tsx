...
// `host.runAgent`'s fixed PermissionPreset never exposes the `workflow_builder__*`
...
export function App() {
	...
	const handleRun = useCallback(
		async (inputs: Record<string, string>) => {
			...
			setRunNodesByWorkflow((prev) => ({ ...prev, [selected.id]: result.nodes }));
	...
	const handleResume = useCallback(
		async (payload: string) => {
			...
			const result = await resumeWorkflow({ url: "", token: null }, runId, payload);
			setRunNodesByWorkflow((prev) => ({ ...prev, [selected.id]: result.nodes }));
			setLastRunByWorkflow((prev) => ({ ...prev, [selected.id]: result }));
		},
