...
const TRIGGER_OPTIONS: { value: TriggerType; label: string }[] = (
	["manual", "schedule", "webhook", "composio"] as TriggerType[]
...
export function TriggerConfig({
	...
	// Free (Band 1) workflows run manually only. Any always-on trigger
	// (schedule / webhook / Composio event) fires in the background — a Band-2
	// "background runs" feature unlocked by a one-time Lifetime license or any
	// subscription. See CAPABILITY_TIERS in @ryu/auth/lib/plans.
	const canSchedule = canUse("local-background-runs");

	const setType = (type: TriggerType) => {
		if (type === trigger.type) {
			return;
		}
		if (type !== "manual" && !canSchedule) {
			requestUpgrade();
			return;
		}
