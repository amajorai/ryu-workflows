...
export interface WorkflowCanvasProps {
	...
	/** Latest host-owned run, including updates after approval or resume. */
...
export function WorkflowCanvas({
	...
	const handleRun = useCallback(
		async (inputs: Record<string, string>, dryRun: boolean) => {
			...
			setRunResult(null);
			try {
				const result = await onRun(inputs, dryRun);
				setRunResult(result);
				...
				>
					...
					<Panel position="top-left">
						<div className="flex w-56 flex-col rounded-xl bg-popover/80 px-3 py-1.5 ring-1 ring-white/10 backdrop-blur">
							<Input
								...
								className="h-6 border-0 bg-transparent p-0 font-medium text-sm shadow-none focus-visible:ring-0"
					...
					<Panel position="top-right">
						<div className="flex items-center gap-1 rounded-full bg-popover/90 p-1 shadow-lg ring-1 ring-white/10 backdrop-blur">
							{saveError ? (
								<span
									className="px-2 text-status-destructive text-xs"
									role="alert"
								>
									{saveError}
								</span>
					...
					{showRun ? (
						<Panel position="bottom-right">
							<RunPanel
								...
								onRun={handleRun}
								result={running || runError ? null : (latestRun ?? runResult)}
								runError={runError}
								running={running}
							/>
						</Panel>
